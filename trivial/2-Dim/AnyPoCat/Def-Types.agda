open import 0-Dim.!quali

module 2-Dim.AnyPoCat.Def-Types where

private module PoCat where
  open import 2-Dim.PoCat.Def-Types.Obj public
  open import 2-Dim.PoCat.Def-Types.Fun public
  open import 2-Dim.PoCat.Def-Gens.Objs public
open PoCat.[Obj]

[Obj] = [Any]
[Fun] = Any.[Fun]

module _ (2A : [2~] [Obj]) where
  [Rel] : [Any]
  [Rel] = (2a : [Dup2] 2A) → PoCat.[Obj]

module _ (2A : [2~] [Obj]) (2R @(R1 > R2) : [2~] [Rel] 2A) where
  Rel-[Fun] : [Any]
  Rel-[Fun] = (2a : [Dup2] 2A) → PoCat.[Fun] (R1 2a > R2 2a)

module _ (A : [Obj]) ((! > R) : [!] [~] [Rel] (2~ A)) where
  Rel-[0Fun] : [Any]
  Rel-[0Fun] = (a : A) → R (2~ a) .Ob

module _ (3A @(A ~ B ~ C) : [3~] [Obj])
    (3R @((R₁₂ × R₂₃) ~ R₁₃) : ([Rel] (A ~ B) [×] [Rel] (B ~ C)) [~] [Rel] (A ~ C)) where
  Rel-[2Fun] : [Any]
  Rel-[2Fun] = (3a @(a ~ b ~ c) : [Dup3] 3A) →
      PoCat.[Fun] (PoCat.Prod2 (R₁₂ (a ~ b) × R₂₃ (b ~ c)) > R₁₃ (a ~ c))

module _ (22A @(2A1 ~ 2A2): [22~] [Obj])
         (2VF @(f1 ~ f2): Any.[Dup|R] (2~ [Fun]) 22A)
         (2HR @(R1 ~ R2): Any.[R|Dup] (2~ [Rel]) 22A) where
  [Rel|Fun] : [Any]
  [Rel|Fun] = (2a @(a1 ~ a2) : [Dup2] 2A1) →
    PoCat.[Fun] (R1 (a1 ~ a2) > R2 (f1 a1 ~ f2 a2))
